...
Конструкторы архивов принимают входной или выходной поток как параметр. Поток используется для сериализации и восстановления данных. Хотя [пример 64.2](#example642) обращается к файлу, другие потоки, такие как stringstream, также могут использоваться.
...
```
Вместо того, чтобы использовать макрос **`BOOST_CLASS_EXPORT`**, [пример 64.13](#example6413) вызывает член функции шаблона **`register_type()`**. Тип регистрации передается в качетсве параметра. Обратите внимание, что **`register_type()`** должна быть вызвана как в **`save()`**, так и в **`load()`**.

Преимуществом **`register_type()`** является то, что необходмо регистрировать только те классы, которые сериализуются. Например, при разработке библиотеки, никто не знает, какие классы раработчик будет использовать для сериализации позже. В то время как макрос **`BOOST_CLASS_EXPORT`** упрощает этот процесс, он сожет оегистрировать типы, которые не будут использоваться при сериализации.
