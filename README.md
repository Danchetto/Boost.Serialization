...
- [Интерфейсные функции для оптимизации](#Интерфейсные-функции-для-оптимизации)
...
[Пример 64.8](#example648) создает объект типа **`animal`** с помощью **`new`** и присваивает его указателю **`a`**. Указатель (не **`*a`**) сериализуется. Boost.Serialization автоматически сериализует объект, на который ссылается указатель **`a`**, а не адрес этого объекта.
...
## Интерфейсные функции для оптимизации

В этом разделе представлены интерфейсные функции (wrapper functions), которые оптимизируют процесс сериализации. Эти функции помечают объекты, чтобы позволить Boost.Serialization применять некоторые методы оптимизации.
...
```
[Пример 64.14](#example6414) использует Boost.Serialization без интерфейсной функции. Пример создает и записывает значения: **`22 serialization::archive 11 0 0 3 0 1 2`** в строку. Используя интерфейсную функцию **`boost::serialization::make_array()`**, значение может сокращено: **`22 serialization::archive 11 0 1 2`**.
